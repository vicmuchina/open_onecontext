...
- Keep long-running OpenCode work aligned with GCC memory discipline via watchman-judged enforcement plus high-confidence suggestion-only memory assistance from prior GCC history, sized by a token budget (default 35% of model context, bounded 30-40%) and split between recency and semantic recall (default 50/50).
